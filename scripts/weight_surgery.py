...
def perform_surgery():
    ...
    # dual_model = YOLO('configs/yolo11_dual_cross_cbam.yaml', task='detect') #Method CMX + CBAM
    dual_model = YOLO("configs/yolo11_dual_rectify_feedback.yaml", task="detect")  # CMX + FeedBack Backbone
    ...
    for key in target_dict.keys():
        ...
        if (
            key.startswith("model.1.")
            or key.startswith("model.2.")
            or key.startswith("model.3.")
            or key.startswith("model.4.")
            or key.startswith("model.5.")
            or key.startswith("model.6.")
            or key.startswith("model.7.")
            or key.startswith("model.8.")
            or key.startswith("model.9.")
            or key.startswith("model.10.")
        ):
            ...
            if source_key in dict_s:
                if target_dict[key].shape == dict_s[source_key].shape:
                    ...
                    print(
                        f"      ⚠️ Lệch size RGB tại {key}: Target {target_dict[key].shape} vs Source {dict_s[source_key].shape}"
                    )

        # === NHÁNH 2: NDVI (Layer 11-20) ===
        elif (
            key.startswith("model.11.")
            or key.startswith("model.12.")
            or key.startswith("model.13.")
            or key.startswith("model.14.")
            or key.startswith("model.15.")
            or key.startswith("model.16.")
            or key.startswith("model.17.")
            or key.startswith("model.18.")
            or key.startswith("model.19.")
            or key.startswith("model.20.")
        ):
            ...
            if source_key in dict_n:
                ...
                if layer_idx == 11 and "conv.weight" in key:
                    ...
                    if target_dict[key].shape == w_compressed.shape:
                        ...
                        transferred_count += 1
                        print("       Đã nén weights 3 kênh -> 1 kênh cho Layer 11 (NDVI Input)")
                    else:
                ...
                # Các layer khác của nhánh NDVI (Copy nguyên xi)
                    ...
                    print(
                        f"       Lệch size NDVI tại {key}: Target {target_dict[key].shape} vs Source {dict_n[source_key].shape}"
                    )
    ...
    # save_path = 'yolo_dual_cross_cbam_pretrain.pt' # CMX + CBAM
    save_path = "yolo_dual_rectify_feedback_pretrain.pt"  # CMX + Feedback backbone
